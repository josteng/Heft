...
struct CommandLineSpecTests {
    ...
    func dispatchCoversSpec() throws {
        ...
        for verb in CommandLineSpec.verbs {
            ...
            // mistaken for a help request. `version` the same way, because it
            // answers to two flag spellings as well as to its own name.
            if verb.name == "help" {
                #expect(combined.contains("CommandLineSpec.wantsHelp"))
                continue
            }
            if verb.name == "version" {
                #expect(combined.contains("CommandLineSpec.wantsVersion"))
                continue
            }
            #expect(
    ...
    /// Every spelling anyone would try. A command line that answers `version`
    /// but not `--version` is one people conclude has no version at all.
    @Test("Version answers to its verb and to both flags")
    func versionSpellings() {
        for spelling in ["version", "--version", "-v"] {
            #expect(CommandLineSpec.wantsVersion([spelling]), "`heft \(spelling)` went unanswered")
        }
        #expect(!CommandLineSpec.wantsVersion([]))
        #expect(!CommandLineSpec.wantsVersion(["open", "--version"]))
        #expect(!CommandLineSpec.wantsVersion(["versions"]))
        // The flag spellings are not verbs, so the wrapper lists them
        // separately; only the verb belongs in the generated case arm.
        #expect(CommandLineSpec.verbNames.split(separator: "\n").contains("version"))
    }

    @Test("The version line says what is missing rather than printing a gap")
    func versionLine() {
        #expect(CommandLineSpec.versionText(marketing: "0.3.2", build: "214")
            == "heft 0.3.2 (build 214)")
        #expect(CommandLineSpec.versionText(marketing: "0.3.2", build: nil) == "heft 0.3.2")
        #expect(CommandLineSpec.versionText(marketing: "0.3.2", build: "") == "heft 0.3.2")
        #expect(CommandLineSpec.versionText(marketing: nil, build: "214")
            == "heft (version unknown)")
        #expect(CommandLineSpec.versionText(marketing: "", build: nil)
            == "heft (version unknown)")
    }
