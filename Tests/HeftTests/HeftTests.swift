...
@Suite("Documented shortcuts")
struct DocumentedShortcutTests {

    /// The README's keyboard table is the first thing anyone reads and there
    /// is nothing to stop it drifting: a shortcut can be renamed, moved to a
    /// different menu group, or — as happened with ⇧⌘E — end up in no menu at
    /// all, and the table goes on promising it.
    @Test("Every shortcut the README promises is declared somewhere")
    func documentedShortcutsExist() throws {
        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        ...
        let app = try read("Sources/Heft/HeftApp.swift")
        let commands = try read("Sources/Heft/Views/AppCommands.swift")

        // Rows of the keyboard table: "| ⇧⌘E | Export as PDF |".
        let pattern = #"^\| (⇧?⌥?⌘[A-Z]) \| "#
        let regex = try NSRegularExpression(pattern: pattern, options: [.anchorsMatchLines])
        let full = NSRange(readme.startIndex..., in: readme)
        let documented = regex.matches(in: readme, range: full).compactMap {
            Range($0.range(at: 1), in: readme).map { range in String(readme[range]) }
        }
        #expect(documented.count >= 10, "found only \(documented) in the table")

        for shortcut in documented {
            // A shortcut is either declared inline in the commands, or named
            // in the `AppCommandShortcut` registry by its display string.
            if commands.contains("display: \"\(shortcut)\"") { continue }

            let letter = String(shortcut.suffix(1)).lowercased()
            var needed = ["\".command"]
            if shortcut.contains("⇧") { needed.append(".shift") }
            if shortcut.contains("⌥") { needed.append(".option") }

            // Find every `keyboardShortcut("x", modifiers: …)` for this letter
            // and check one carries exactly the modifiers claimed.
            let declarations = try NSRegularExpression(
                pattern: "keyboardShortcut\\(\"\(letter)\", modifiers: ([^)]*\\)?)"
            )
            let matches = declarations.matches(in: app, range: NSRange(app.startIndex..., in: app))
            let satisfied = matches.contains { match in
                guard let range = Range(match.range(at: 1), in: app) else { return false }
                let modifiers = String(app[range])
                let wantsShift = shortcut.contains("⇧")
                let wantsOption = shortcut.contains("⌥")
                return modifiers.contains(".shift") == wantsShift
                    && modifiers.contains(".option") == wantsOption
            }
            #expect(
                satisfied,
                "the README promises \(shortcut) but nothing declares it"
            )
            _ = needed
        }
    }
