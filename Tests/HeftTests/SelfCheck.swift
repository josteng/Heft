...
public enum SelfCheck {
    ...
    public static func run() -> Result {
        ...
        // A note can end up with more than one marker: the template puts one
        // there, and another arrives with any whole body pasted or accepted
        // from an agent proposal that carried its own. Entries have to keep
        // going where the previous ones went — the end of the log — rather
        // than jumping to the middle of the note the moment a second appears.
        let twoMarkers = """
        # 2026-08-07
        ...
        expect(
            try DailyNoteCapture.contents(
                byCapturing: "Later entry", in: twoMarkers, at: day, calendar: cal
            ),
            """
            # 2026-08-07
            ...
            ---
            """,
            "a duplicated marker does not move the insertion point backwards"
        )
        } catch {
            expectTrue(false, "capturing into a note with two markers threw: \(error)")
        }
