...
extension VimText {
    ...
    func delimitedObject(
        ...
        return delimitedRange(pairs[count - 1], at: location, quoted: false, around: around)
    }

    // MARK: - Quotes, straight and curly

    /// A quote object's candidate delimiters. `i"` has to recognise more than
    /// the character that was typed: Smart Typography rewrites `"` to `“ ”`
    /// and `'` to `‘ ’` as the note is written, so by the time the caret is
    /// inside the quotation the straight form is no longer in the file.
    public struct QuoteForm: Sendable {
        public var opening: Character
        public var closing: Character

        public init(_ opening: Character, _ closing: Character) {
            self.opening = opening
            self.closing = closing
        }
    }

    /// The best of `forms` at `location`: the innermost quotation containing
    /// the caret, or failing that the nearest one after it — the same order of
    /// preference a single quote form already uses, applied across all of them
    /// so that a line mixing straight and curly quotes still picks the right
    /// one.
    func quoteObject(at location: Int, forms: [QuoteForm], around: Bool) -> NSRange? {
        var containing: (left: Int, right: Int)?
        var following: (left: Int, right: Int)?
        for form in forms {
            guard let pair = quotePair(at: location, form: form) else { continue }
            if pair.left <= location, location <= pair.right {
                if containing == nil || pair.left > containing!.left { containing = pair }
            } else if pair.left > location, following == nil || pair.left < following!.left {
                following = pair
            }
        }
        guard let pair = containing ?? following else { return nil }
        return delimitedRange(pair, at: location, quoted: true, around: around)
    }

    /// Quote objects are line-scoped, like Vim's own. A form whose two ends
    /// differ is still scanned that way rather than as a bracket: `“` opens and
    /// `”` closes, so a stray `’` apostrophe in `it’s` never pairs with
    /// anything and cannot swallow half a sentence.
    private func quotePair(at location: Int, form: QuoteForm) -> (left: Int, right: Int)? {
        let line = lineContentRange(at: location)
        let opening = (String(form.opening) as NSString).character(at: 0)
        let closing = (String(form.closing) as NSString).character(at: 0)
        if opening == closing {
            var delimiters: [Int] = []
            var p = line.location
            while p < NSMaxRange(line) {
                if value.character(at: p) == opening, !isEscapedQuote(at: p) { delimiters.append(p) }
                p = nextCharacter(from: p)
            }
            guard delimiters.count >= 2 else { return nil }
            // One character cannot say whether it opens or closes, so Vim
            // counts: a quote at an even index from the start of the line
            // opens, one at an odd index closes.
            if let index = delimiters.firstIndex(of: location) {
                guard index.isMultiple(of: 2) else {
                    return (delimiters[index - 1], delimiters[index])
                }
                guard index + 1 < delimiters.count else { return nil }
                return (delimiters[index], delimiters[index + 1])
            }
            // Off a quote, the object is bounded by the quote before the caret
            // and the one after it — so between `"one"` and `"two"`, `ci"`
            // changes the gap rather than jumping to the next quotation.
            guard let after = delimiters.firstIndex(where: { $0 > location }) else { return nil }
            return after == 0
                ? (delimiters[0], delimiters[1])
                : (delimiters[after - 1], delimiters[after])
        }
        ...
        while p < NSMaxRange(line) {
            ...
            if character == opening, open == nil {
                open = p
            } else if character == closing, let left = open {
                open = nil
    ...
    private func isEscapedQuote(at location: Int) -> Bool {
        var backslashes = 0
        var p = location
        while p > 0 {
            p -= 1
