...
///
/// - **Edit, Write and NotebookEdit are denied inside the vault**, by path
///   rather than by tool name. Denying the tools outright would also stop the
///   agent writing the scratch file the guide's own workflow uses
...
public enum AgentPermissions {
    ...
    public static let deny = ["Edit(**)", "Write(**)", "NotebookEdit(**)"]
    ...
    public static func merged(into existing: String?) -> String {
        ...
        for (key, wanted) in [("allow", allow), ("deny", deny)] {
            var rules = permissions[key] as? [String] ?? []
    ...
    public static func isSatisfied(by existing: String?) -> Bool {
        ...
        return allow.allSatisfy(allowed.contains) && deny.allSatisfy(denied.contains)
    }
